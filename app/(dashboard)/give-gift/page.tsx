...
import { giftService, GiftRequest, SubscriptionPlan } from "./gift-service";
...
export default function GiveGiftScreen() {
  ...
  const giftOptions = subscriptionPlans.map((plan) => ({
    id: String(plan.id),
    name: `${plan.name} Subscription`,
    description: `${plan.memoryBookCount} book${plan.memoryBookCount > 1 ? "s" : ""} • ${plan.highlightReelCount} reel${plan.highlightReelCount > 1 ? "s" : ""}`,
    price: plan.price,
  }));
  const [formData, setFormData] = useState<Pick<GiftRequest, "senderName" | "senderEmail" | "senderPhone" | "recipientName" | "recipientEmail" | "recipientPersonalMessage" | "videoMessageUrl" | "deliveryDate">>({
    // Sender Details
    senderName: "",
    senderEmail: "",
    senderPhone: "",
  ...
  useEffect(() => {
    ...
    const fetchSubscriptionPlans = async () => {
      try {
        const response = await giftService.getSubscriptionPlans();
        if (isMounted) {
  ...
  useEffect(() => {
    if (!selectedSubscription && giftOptions.length > 0) {
      setSelectedSubscription(giftOptions[0].id);
    }
  }, [giftOptions, selectedSubscription]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (giftCardType === "physical" && !selectedDesign) {
      toast({
        title: "Select a gift card design",
        description: "Please choose a design before proceeding with a physical gift card.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const payload: GiftRequest = {
        subscriptionPlanId: Number(selectedSubscription),
        giftCardType,
        ...
        recipientPersonalMessage: formData.recipientPersonalMessage.trim(),
        videoMessageUrl: formData.videoMessageUrl,
        state: selectedState,
        deliveryDate: formData.deliveryDate || null,
        couponCode: couponCode.trim() || null,
      };

      await giftService.postGift(payload);
      ...
      toast({
        title: "Unable to place gift order",
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          >
            ...
            >
              <SelectTrigger className="h-12 w-full rounded-lg border-border bg-card p-4 text-left hover:border-vault-teal/60 transition-colors">
                {selectedPlan ? (
                  <div className="flex w-full items-center justify-between gap-3 py-6">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-foreground">{selectedPlan.name}</div>
                      <div className="truncate text-xs text-muted-foreground">{selectedPlan.description}</div>
                    </div>
                    <div className="shrink-0 text-sm font-semibold text-vault-teal">
              ...
              <SelectContent className="rounded-lg p-2">
                {giftOptions.map((option) => (
                  <SelectItem key={option.id} value={option.id} className="rounded-md px-3 py-3">
                    <div className="flex w-full items-center justify-between gap-4">
                      <div className="min-w-0">
                        <div className="font-medium text-foreground">{option.name}</div>
                        <div className="text-xs text-muted-foreground mt-0.5">{option.description}</div>
                      </div>
                      <div className="ml-4 shrink-0 font-semibold text-vault-teal">
          ...
          >
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                <Label htmlFor="recipientPersonalMessage" className="text-xs text-muted-foreground">Personal Message</Label>
                <div className="relative">
                  <MessageSquare className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" />
                  <Textarea
                    id="recipientPersonalMessage"
                    name="recipientPersonalMessage"
                    placeholder="Write a heartfelt message to accompany your gift..."
                    value={formData.recipientPersonalMessage}
                    onChange={handleChange}
          ...
          >
            ...
            <Button
              type="submit"
              disabled={isSubmitting || !selectedState || (giftCardType === "physical" && !selectedDesign)}
