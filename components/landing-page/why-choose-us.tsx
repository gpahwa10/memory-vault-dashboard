...
export const WhyChooseUs = () => {
    return (
        <div className="bg-[#1D453A] flex w-full flex-col items-center justify-between gap-[64px] px-4 py-12 sm:px-6 sm:py-16 md:px-10 md:py-20 lg:px-14 lg:py-24 xl:px-20 xl:py-28 2xl:px-32 2xl:py-24 min-[1920px]:px-[240px] min-[1920px]:py-[116px]">
            <div className="flex w-full max-w-[1479px] flex-col items-start gap-6 md:flex-row md:items-start md:justify-between md:gap-[32px]">
                <h2 className={`${gtSuperDisplay.className} text-5xl font-400 text-white`}>Why Choose <span className="text-[#CAA64A]">Memory Vault</span></h2>
                <p className={`${jost.className} flex max-w-[592px] text-2xl font-400 text-white`}>Every chapter of our process is designed to delight you — from premium materials to smart technology.</p>
            </div>
            ...
            <div className="flex w-full max-w-[1479px] flex-wrap items-start justify-center gap-[32px] xl:flex-nowrap xl:justify-between">
...
const WhyChooseUsItem = ({ item }: { item: (typeof whyChooseUsItems)[number] }) => {
    return (
        <div className="flex h-[331px] w-[345.75px] flex-col items-start justify-center gap-6 rounded-[12px] bg-[#13352B] p-[42px]">
            <img src={item.image} alt={item.title} className="w-full h-auto object-cover max-w-[64px] max-h-[64px]" />
            <h3 className={`${jost.className} text-xl font-500 text-white`}>{item.title}</h3>
            <p className={`${jost.className} text-base font-400 text-white`}>{item.description}</p>
        </div>
    )
}
