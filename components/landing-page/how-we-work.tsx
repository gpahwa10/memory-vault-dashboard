...
export const HowWeWork = () => {
    return (
        <div className="flex flex-col items-center justify-center bg-[#EDE9DF] px-4 py-12 sm:px-6 sm:py-16 md:px-10 md:py-20 lg:px-14 lg:py-24 xl:px-20 xl:py-28 2xl:px-32 2xl:py-32 min-[1920px]:px-[240px] min-[1920px]:py-[124px]">
                <h2 className="max-w-4xl text-center text-3xl font-500 leading-tight text-[#12473A] sm:text-4xl lg:text-5xl">
                    From Memories <span className="text-[#CAA64A]">to Masterpiece</span>
                </h2>
            <p className={`${jost.className} mt-6 mb-8 max-w-[740px] px-1 text-center text-base font-400 text-[#615F5A] sm:text-lg md:text-xl lg:mb-10 lg:mt-6 lg:text-2xl`}>
            ...
            <Button className="mt-8 inline-flex items-center gap-2 rounded-full border-0 bg-[#CAA64A] px-4 py-2 text-sm font-400 text-black hover:bg-[#b89442] lg:mt-10">
    ...
    return (
        <div className="mx-auto flex h-full min-w-0 w-full max-w-[460px] flex-col rounded-[20px] border border-solid border-[#E3DED0] bg-white p-5 sm:p-6 md:mx-0 md:max-w-none lg:p-8">
            <div className="flex w-full min-w-0 flex-row items-start justify-between gap-3">
                <h3 className={`${gtSuperDisplay.className} min-w-0 flex-1 text-left text-xl font-500 leading-snug text-[#12473A] sm:text-2xl`}>
                    {step.title}
                </h3>
                <p
                    className={`${jost.className} shrink-0 text-5xl leading-none font-bold tabular-nums text-[#CAA64A]/20 sm:text-6xl lg:text-[80px]`}
                    aria-hidden
                >
                    {step.id}
                </p>
            ...
            <p className={`${jost.className} mt-6 text-left text-base font-400 leading-relaxed text-[#615F5A] sm:text-lg lg:mt-8`}>
                {step.description}
            </p>
        </div>
    )
